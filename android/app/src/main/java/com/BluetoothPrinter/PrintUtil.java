...
public class PrintUtil {
    ...
    public static void printTest(BluetoothSocket bluetoothSocket, Bitmap bitmap, String shop_name, JSONObject data) {

        try {
            PrintUtil pUtil = new PrintUtil(bluetoothSocket.getOutputStream(), "GBK");
            // 店铺名 居中 放大
            pUtil.printAlignment(1);
            pUtil.printLargeText(shop_name);
            pUtil.printLine();
            pUtil.printAlignment(0);
            pUtil.printLine();

            pUtil.printTwoColumn("时间:", data.getString("date").replaceAll("[\\t\\n\\r]", ""));
            pUtil.printLine();

            pUtil.printTwoColumn("订单号:", data.getString("order_number").replaceAll("[\\t\\n\\r]", ""));
            pUtil.printLine();

            pUtil.printTwoColumn("流水号:", data.getString("shop_every_id").replaceAll("[\\t\\n\\r]", ""));
            pUtil.printLine();

            pUtil.printTwoColumn("付款人:", data.getString("user_name").replaceAll("[\\t\\n\\r]", ""));
            pUtil.printLine();
            ...
            JSONArray ls = new JSONArray( data.getString("order_information"));
            for (int i = 0; i < ls.length(); i++){
                double order_price = ls.getJSONObject(i).getInt("total")/100.0;
                String goods_name = ls.getJSONObject(i).getString("goods_name").replaceAll("[\\t\\n\\r]", "");
                String goods_type_name = ls.getJSONObject(i).getString("goods_type_name").replaceAll("[\\t\\n\\r]", "");
                pUtil.printThreeColumn(goods_name+" "+goods_type_name, ls.getJSONObject(i).getString("number"), String.format("%.2f",order_price));
                //pUtil.printThreeColumn("红烧茄子(大) ", "1", "12.00");
                //pUtil.printThreeColumn("青椒炒牛肉（8:00） ", "2", "16.00");
            ...
            String[] order_type = {"","桌上点餐","预定点餐","线上点餐","前台点餐"};

            pUtil.printTwoColumn("订单类型:", order_type[data.getInt("order_type")]);
            pUtil.printLine();

            String[] payment_status ={"未支付","线上支付","预留","已退款","商家结算"};
            pUtil.printTwoColumn("付款方式:", payment_status[data.getInt("payment_status")]);
            pUtil.printLine();

            double basic_price = data.getInt("basic_price")/100.0;
            pUtil.printTwoColumn("基本金额:", String.format("%.2f",basic_price));
            pUtil.printLine();

            if(data.getInt("order_type")==2 || data.getInt("order_type")==3){
                double packing_fee = data.getInt("packing_fee")/100.0;
                pUtil.printTwoColumn("打包费:", String.format("%.2f",packing_fee));
                pUtil.printLine();
            }

            if(data.getInt("order_type")==3){
                double takeout_fee = data.getInt("takeout_fee")/100.0;
                pUtil.printTwoColumn("外卖费:", String.format("%.2f",takeout_fee));
                pUtil.printLine();
            }

            double order_price = data.getInt("order_price")/100.0;
            pUtil.printTwoColumn("实收金额:", String.format("%.2f",order_price));
            pUtil.printLine();

            pUtil.printDashLine();

            pUtil.printText("电话:"+data.getString("phone_number").replaceAll("[\\t\\n\\r]", ""));
            pUtil.printLine();

            if(data.getInt("order_type")==1){
                pUtil.printAlignment(1);
                pUtil.printLargeText(data.getString("address").replaceAll("[\\t\\n\\r]", ""));
                pUtil.printLine();
                pUtil.printAlignment(0);
                pUtil.printLine();
            }else {
                pUtil.printText("地址:"+ data.getString("address").replaceAll("[\\t\\n\\r]", ""));
                pUtil.printLine();
            }

            pUtil.printLine();
            pUtil.printText("备注:"+data.getString("remark").replaceAll("[\\t\\n\\r]", ""));
            pUtil.printLine();

            pUtil.printBitmap(bitmap);
            pUtil.printAlignment(1);
            pUtil.printText("订单标识        广理商业街");
